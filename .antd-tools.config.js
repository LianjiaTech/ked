...
// We need compile additional content for ked user
function finalizeCompile() {
  if (fs.existsSync(path.join(__dirname, './lib'))) {
    ...
    // Build a entry less file to dist/ked.less
...
function finalizeDist() {
  if (fs.existsSync(path.join(__dirname, './dist'))) {
    // Build less entry file: dist/ked.less
    fs.writeFileSync(
      path.join(process.cwd(), 'dist', 'ked.less'),
      '@import "../lib/style/index.less";\n@import "../lib/style/components.less";',
    );

    // eslint-disable-next-line
    console.log('Built a entry less file to dist/ked.less');
  }
}

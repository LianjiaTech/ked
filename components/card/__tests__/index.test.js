...
describe('Card', () => {
  ...
  it('warning', () => {
    ...
    expect(warnSpy).toHaveBeenCalledWith(
      'Warning: [ked: Card] `noHovering` is deprecated, you can remove it safely or use `hoverable` instead.',
    );
    mount(<Card noHovering={false}>xxx</Card>);
    expect(warnSpy).toHaveBeenCalledWith(
      'Warning: [ked: Card] `noHovering={false}` is deprecated, use `hoverable` instead.',
    );
    warnSpy.mockRestore();
  });

...
export default class Select<T = SelectValue> extends React.Component<SelectProps<T>, {}> {
  ...
  renderSelect = ({
    ...
    const handleOnChange = (value: T, option: React.ReactElement<any> | React.ReactElement<any>[]):void => {
      if (!rest.onChange) return

      let tmp: any = ''

      if (rest.labelInValue && value && Array.isArray(value)) {
        tmp = value.map((v) => ({ ...v, value: v.key }))
      } else if (rest.labelInValue && value && typeof value === 'object') {
        tmp = { ...value, value: (value as any).key }
      } else {
        tmp = value
      }

      rest.onChange(tmp, option)
    }

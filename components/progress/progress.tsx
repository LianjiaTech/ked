...
import CircleException from "./circle-exception";
import CircleFinished from "./circle-finished";
...
export default class Progress extends React.Component<ProgressProps> {
  ...
  renderProcessInfo(prefixCls: string, progressStatus: (typeof ProgressStatuses)[number]) {
    const { showInfo, format, type, percent, successPercent, width = 120 } = this.props;
    if (!showInfo) return null;

    let text;
    const iconWidth = Math.round(width * 0.3)
    ...
    if (format || (progressStatus !== 'exception' && progressStatus !== 'success')) {
      ...
      text = type === 'circle'
        ? (<CircleException prefixCls={prefixCls} iconWidth={iconWidth} />)
        : (<Icon type={`exclamation${iconType}`} theme={type === 'line' ? 'filled' : 'outlined'} />);
    } else if (progressStatus === 'success') {
      text = type === 'circle'
        ? (<CircleFinished prefixCls={prefixCls} iconWidth={iconWidth} />)

...
describe('Transfer.Customize', () => {
  ...
  it('should warning use body', () => {
    ...
    expect(errorSpy).toHaveBeenCalledWith(
      'Warning: [ked: Transfer] `body` is internal usage and will bre removed, please use `children` instead.',
    );
  });
        ...
        <Transfer
          ...
          body={({ handleSelect, handleSelectAll }) => {
            if (init) {
              handleSelect('', true);
              expect(errorSpy).toHaveBeenCalledWith(
                'Warning: [ked: Transfer] `handleSelect` will be removed, please use `onSelect` instead.',
              );
              errorSpy.mockReset();
              handleSelectAll([], true);
              expect(errorSpy).toHaveBeenCalledWith(
                'Warning: [ked: Transfer] `handleSelectAll` will be removed, please use `onSelectAll` instead.',
              );
            }
            init = false;
            return null;
